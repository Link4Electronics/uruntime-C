...
```
git clone https://github.com/Link4Electronics/uruntime-C.git && cd uruntime
```

* **Compile a binary**

Requires: cmake 3.25+, gcc/clang, zstd-dev, xxhash-dev.

```
# Fetch embedded tool binaries for your arch
./fetch_assets.sh x86_64

# Build (static musl recommended for portability)
cmake -S . -B build -DCMAKE_BUILD_TYPE=MinSizeRel -DURUNTIME_STATIC=ON
...
|--------|---------|-------------|
| `URUNTIME_APPIMAGE` | OFF | AppImage mode (vs RunImage) |
| `URUNTIME_SQUASHFS` | ON | SquashFS support |
| `URUNTIME_DWARFS` | ON | DwarFS support |
| `URUNTIME_LITE` | OFF | Lite mode (no mksquashfs/dwarfsck/mkdwarfs) |
| `URUNTIME_STATIC` | ON | Fully static link |

Static builds require `zstd-static` and `xxhash-static` packages (provided by Alpine, need manual install on other distros). Set `-DURUNTIME_STATIC=OFF` to link shared.

Multi-arch builds are handled by CI via Docker + QEMU (see [ci.yml](.github/workflows/ci.yml)).

* Or take an already precompiled from the [releases](https://github.com/Link4Electronics/uruntime-C/releases)

### **Runtime configuration via environment variables:**

| Variable | Values | Description |
|----------|--------|-------------|
| `URUNTIME_EXTRACT` | 0-3 | 0=FUSE only, 1=extract only, 2=FUSE fallback to extract, 3=auto (default) |
| `URUNTIME_CLEANUP` | 0-1 | 0=keep extracted dir, 1=clean up (default) |
| `URUNTIME_UNSHARE` | 0-1 | 0=no unshare (default), 1=use user+mount namespaces |
| `URUNTIME_MOUNT` | 0-3 | Mount reuse/unmount strategy (default: 3 = reuse + 5s idle unmount) |

Default values are compiled in but can be overridden at runtime via environment variables.
